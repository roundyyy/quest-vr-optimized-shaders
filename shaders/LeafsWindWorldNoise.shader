Shader "Roundy/Vegetation/LeafsWindWorldNoise" {
    Properties {
        ...
        _NoiseTex ("World Noise Texture", 2D) = "white" {}
    ...
    SubShader {
        ...
        Pass {
            ...
            v2f vert(appdata v) {
                v2f o = (v2f)0; // Initialize all fields to 0
            ...
            half4 frag(v2f i) : SV_Target {
                ...
                // Sample noise texture in world space
                float2 worldUV = i.worldPos.xz * _NoiseScale;
                half4 noise = tex2D(_NoiseTex, worldUV);
                
                // Apply noise to color
                col.rgb = lerp(col.rgb, col.rgb * noise.rgb, _NoisePower);
                    ...
                    col.rgb *= lerp(1.0h, DecodeLightmap(bakedColorTex) * 2.0h, _LightmapPower);
                #endif
